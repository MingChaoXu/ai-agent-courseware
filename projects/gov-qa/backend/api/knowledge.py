...
@router.post("/knowledge/upload", response_model=KbUploadResponse)
async def upload_text(title: str = Form(...), content: str = Form(...), chunk_size: int = Form(500)):
    """
    Upload text content to the knowledge base.
    The text will be split into chunks, embedded, and indexed.
    """
    if not knowledge_base:
        raise HTTPException(status_code=503, detail="Knowledge base not initialized")
    try:
        result = knowledge_base.upload_text(title=title, content=content, chunk_size=chunk_size)
...
async def upload_file(file: UploadFile = File(...), chunk_size: int = Form(500)):
    ...
    Supports .txt, .md, .json (FAQ format) files.
    """
    if not knowledge_base:
        raise HTTPException(status_code=503, detail="Knowledge base not initialized")
