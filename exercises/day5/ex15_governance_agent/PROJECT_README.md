...
4个Agent协作：event_entry_agent(事件录入) + legal_consultation_agent(法律咨询,条件触发) + brief_generation_agent(通报生成) + alert_agent(预警处置)。本文件是代码实现的技术文档，包含架构设计、API说明和代码细节。

## 项目结构

```
ex15_governance_agent/
├── backend/
│   ├── main.py
│   ├── config.py
│   ├── agent/agent.py
